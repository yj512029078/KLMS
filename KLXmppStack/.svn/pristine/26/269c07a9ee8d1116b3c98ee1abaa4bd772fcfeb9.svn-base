...
namespace gloox
{
  ...
  SIProfileFT::SIProfileFT( ClientBase* parent, SIProfileFTHandler* sipfth, SIManager* manager, SOCKS5BytestreamManager* s5Manager )
    : m_parent( parent ), 
	  m_manager( manager ), 
	  m_handler( sipfth ),
      m_socks5Manager( s5Manager ), 
	  m_delManager( false ),
  ...
  void SIProfileFT::acceptFT( const JID& to, const std::string& sid, StreamType type, const JID& from )
  {
    ...
    switch( type )
    {
      ...
      case FTTypeIBB:
        ...
        if( m_handler )
        {
          InBandBytestream* ibb = new InBandBytestream( m_parent, m_parent->logInstance(), to, from ? from : m_parent->jid(), sid );
  ...
  void SIProfileFT::declineFT( const JID& to, const std::string& sid, SIManager::SIError reason, const std::string& text )
  {
  ...
  void SIProfileFT::handleSIRequestResult( const JID& from, const JID& to, const std::string& sid, const SIManager::SI& si )
  {
    if( si.tag2() )
    {
      const DataForm df( si.tag2()->findChild( "x", XMLNS, XMLNS_X_DATA ) );

...
namespace gloox
{
  ...
  ClientBase::ClientBase( const std::string& ns, const std::string& server, int port )
    : m_connection( 0 ), 
      ...
      m_messageSessionHandlerChat( 0 ), 
	  m_messageSessionHandlerGroupchat( 0 ),
      m_messageSessionHandlerHeadline( 0 ), 
	  m_messageSessionHandlerNormal( 0 ),
      m_parser( this ), 
	  m_seFactory( 0 ), 
	  m_authError( AuthErrorUndefined ),
      m_streamError( StreamErrorUndefined ), 
	  m_streamErrorAppCondition( 0 ),
      m_selectedSaslMech( SaslMechNone ), 
	  m_autoMessageSession( false )
  {
    init();
  }

  ClientBase::ClientBase( const std::string& ns, const std::string& password, const std::string& server, int port )
    : m_connection( 0 ), 
	  m_encryption( 0 ), 
	  m_compression( 0 ), 
	  m_disco( 0 ), 
...
//         etc... see gssapi-sasl-draft.txt
      ...
      case SaslMechNTLM:
      {
        ...
        {
          identity.User = (unsigned short*)m_jid.username().c_str();
          identity.UserLength = (unsigned long)m_jid.username().length();
          identity.Domain = (unsigned short*)m_ntlmDomain.c_str();
          identity.DomainLength = (unsigned long)m_ntlmDomain.length();
          identity.Password = (unsigned short*)m_password.c_str();

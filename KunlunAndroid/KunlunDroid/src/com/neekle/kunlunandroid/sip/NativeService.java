...
//yj
//临时关键代码 ScreenAV.receiveCall 被注释
...
public class NativeService extends NgnNativeService {
	...
	public void onStart(Intent intent, int startId) {
		...
		mBroadcastReceiver = new BroadcastReceiver() {
			...
			public void onReceive(Context context, Intent intent) {
				...
				if (NgnRegistrationEventArgs.ACTION_REGISTRATION_EVENT
					...
					default:
						...
						if (mEngine.getSipService().isRegistered()) {
							// mEngine.showAppNotif(
							// bTrying ? R.drawable.bullet_ball_glass_grey_16
							// : R.drawable.bullet_ball_glass_green_16,
							// null);
							KunlunApplication.acquirePowerLock();
						} else {
							// mEngine.showAppNotif(
							// bTrying ? R.drawable.bullet_ball_glass_grey_16
							// : R.drawable.bullet_ball_glass_red_16,
							// null);
							KunlunApplication.releasePowerLock();
						}
						break;
					}
				}
				...
				if (NgnMessagingEventArgs.ACTION_MESSAGING_EVENT.equals(action)) {
					...
					case INCOMING:
						...
						NgnHistorySMSEvent event = new NgnHistorySMSEvent(
								remoteParty, StatusType.Incoming);
				...
				// MSRP chat Events
				// For performance reasons, file transfer events will be handled
				...
				if (NgnMsrpEventArgs.ACTION_MSRP_EVENT.equals(action)) {
					...
					switch (args.getEventType()) {
					case DATA:
						final NgnMsrpSession session = NgnMsrpSession
								.getSession(args.getSessionId());
						if (session == null) {
							Log.e(TAG, "Failed to find MSRP session with id="
									+ args.getSessionId());
							return;
						}
						...
						NgnHistorySMSEvent event = new NgnHistorySMSEvent(
								NgnUriUtils.getUserName(session
										.getRemotePartyUri()),
								StatusType.Incoming);
						event.setContent(content == null ? NgnStringUtils
								.nullValue() : new String(content));
						mEngine.getHistoryService().addEvent(event);
						// mEngine.showSMSNotif(R.drawable.sms_25,
						// "New message");
						break;
				...
				// Invite Events
					...
					case INPROGRESS:
						if (NgnMediaType.isAudioVideoType(mediaType)) {
							...
							// getString(R.string.string_call_outgoing));
						} else if (NgnMediaType.isFileTransfer(mediaType)) {
							// mEngine.refreshContentShareNotif(R.drawable.image_gallery_25);
						} else if (NgnMediaType.isChat(mediaType)) {
							// mEngine.refreshChatNotif(R.drawable.chat_25);
					...
					case EARLY_MEDIA:
						if (NgnMediaType.isAudioVideoType(mediaType)) {
							// mEngine.showAVCallNotif(R.drawable.phone_call_25,
							// getString(R.string.string_incall));
							mEngine.getSoundService().stopRingBackTone();
							mEngine.getSoundService().stopRingTone();
						} else if (NgnMediaType.isFileTransfer(mediaType)) {
							// mEngine.refreshContentShareNotif(R.drawable.image_gallery_25);
	...
	private void switchTelVoiceActivity(String sessionId) {
		Context context = KunlunApplication.getContext();
		Class<? extends Activity> targetActivity = TelVoiceActivity.class;
		doSwitchActivity(context, targetActivity, sessionId);
	}

	private void switchTelVideoActivity(String sessionId) {
		Context context = KunlunApplication.getContext();
		Class<? extends Activity> targetActivity = TelVideoActivity.class;
		doSwitchActivity(context, targetActivity, sessionId);
	}

	private void doSwitchActivity(Context context,
			Class<? extends Activity> targetActivity, String sessionId) {
		Bundle bundle = new Bundle();
		bundle.putString(SipConstants.INTENT_AV_SESSION_ID_KEY, sessionId);
		String telAction = TelAction.INCOMING;

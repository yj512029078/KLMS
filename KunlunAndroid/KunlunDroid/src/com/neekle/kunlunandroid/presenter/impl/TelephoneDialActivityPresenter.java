...
import com.neekle.kunlunandroid.screens.TelephoneDialActivity;
...
public class TelephoneDialActivityPresenter {
	...
	public TelephoneDialActivityPresenter(ITelephoneDialActivity view,
			Context context) {
		mIView = view;
		mContext = context;
	}
	...
	// 暂时先放在这里，再想想
	public void doToConstrutPrefix(String prefix) {
		char[] charArray = prefix.toCharArray();
		int length = charArray.length;

		ArrayList<String> arrayList = new ArrayList<String>();
		for (int i = 0; i < length; i++) {
			String key = charArray[i] + Constants.EMPTY_STRING;
			String value = TelephoneDialActivity.mHashMap.get(key);

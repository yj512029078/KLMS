...
/**
 * Next Generation Network Engine. This is the main entry point to have access
 * to all services (SIP, XCAP, MSRP, History, ...). Anywhere in the code you can
 * get an instance of the engine by calling @ref getInstance() function.
 */
public class NgnEngine {
	...
	// This function will be renamed as "initialize()" when "initialize()" get
	// removed
	private static void initialize2() {
		// do not add try/catch to let the app die if libraries are missing or
		// incompatible
		if (!sInitialized) {
			// See 'http://code.google.com/p/imsdroid/issues/detail?id=197' for
			// more information
			// Load Android utils library (required to detect CPU features)
			boolean haveLibUtils = new File(String.format("%s/%s",
					NgnEngine.LIBS_FOLDER, "libutils_armv5te.so")).exists();
			if (haveLibUtils) { // only "armeabi-v7a" comes with "libutils.so"
				System.load(String.format("%s/%s", NgnEngine.LIBS_FOLDER,
						"libutils_armv5te.so"));
			...
			// If OpenSL ES is supported and know to work on current device then
			// used it
			if (NgnApplication.isSLEs2KnownToWork()) {
				final String pluginPath = String.format("%s/%s",
						NgnEngine.LIBS_FOLDER, "libplugin_audio_opensles.so");

				// returned value is the number of registered add-ons (2 = 1
				// consumer + 1 producer)
				if (MediaSessionMgr.registerAudioPluginFromFile(pluginPath) < 2) {
					// die if cannot load add-ons
					throw new RuntimeException(
							"Failed to register audio plugin with path="
									+ pluginPath);
				}
	...
	// This function is deprecated and you no longer need to call it. Also, do
	// not load the native libs in your app
	...
	/**
	 * Default constructor for the NGN engine. You should never call this
	 * function from your code. Instead you should use @ref getInstance().
	 * 
	 * @sa @ref getInstance()
	 */
	protected NgnEngine() {
		...
		if (applicationContext != null) {
			mNotifManager = (NotificationManager) applicationContext
		...
		SipStack.setCodecPriority(tdav_codec_id_t.tdav_codec_id_speex_wb,
				prio++);
		SipStack.setCodecPriority(tdav_codec_id_t.tdav_codec_id_speex_uwb,
				prio++);
		SipStack.setCodecPriority(tdav_codec_id_t.tdav_codec_id_speex_nb,
				prio++);
		...
		SipStack.setCodecPriority(tdav_codec_id_t.tdav_codec_id_amr_nb_oa,
				prio++);
		SipStack.setCodecPriority(tdav_codec_id_t.tdav_codec_id_amr_nb_be,
				prio++);
		...
		SipStack.setCodecPriority(tdav_codec_id_t.tdav_codec_id_mp4ves_es,
				prio++);
		...
		// Profile
		MediaSessionMgr.defaultsSetProfile(tmedia_profile_t
				.valueOf(configurationService.getString(
		...
		final boolean aec = configurationService.getBoolean(
				NgnConfigurationEntry.GENERAL_AEC,
				NgnConfigurationEntry.DEFAULT_GENERAL_AEC);
		final boolean echo_tail_adaptive = configurationService.getBoolean(
				NgnConfigurationEntry.GENERAL_USE_ECHO_TAIL_ADAPTIVE,
				NgnConfigurationEntry.DEFAULT_GENERAL_USE_ECHO_TAIL_ADAPTIVE);
		final boolean vad = configurationService.getBoolean(
				NgnConfigurationEntry.GENERAL_VAD,
				NgnConfigurationEntry.DEFAULT_GENERAL_VAD);
		final boolean nr = configurationService.getBoolean(
				NgnConfigurationEntry.GENERAL_NR,
				NgnConfigurationEntry.DEFAULT_GENERAL_NR);
		final int echo_tail = configurationService.getInt(
		...
		Log.d(TAG, "Configure AEC[" + aec + "/" + echo_tail
				+ "] AEC_TAIL_ADAPT[" + echo_tail_adaptive
				+ "] NoiseSuppression[" + nr + "], Voice activity detection["
				+ vad + "]");
		...
		if (aec) {
			...
			// When using Speex-DSP, any number is valid but you should choose a
			// multiple of 20ms
			// In all cases this value will be updated per session if adaptive
			// echo tail option is enabled
		...
		// /!\IMPORTANT: setting the Jitter buffer max late to (0) cause
		// "SIGFPE" error in SpeexDSP function
		// "jitter_buffer_ctl(JITTER_BUFFER_SET_MAX_LATE_RATE)"
		// This only happen when the audio engine is dynamically loaded from
		// shared library (at least on Galaxy Nexus)
		...
		// opensl-es playback_rates: 8000, 11025, 16000, 22050, 24000, 32000,
		// 44100, 64000, 88200, 96000, 192000
		// webrtc aec record_rates: 8000, 16000, 32000
		MediaSessionMgr.defaultsSetOpusMaxCaptureRate(16000);// /!\IMPORTANT:
																// only 8k and
																// 16k will work
																// with WebRTC
																// AEC
	...
	public synchronized boolean start() {
		...
		if (success) {
			...
			Class<? extends NgnNativeService> serviceClass = getNativeServiceClass();
			String name = serviceClass.getName();
			Intent intent = new Intent(NgnApplication.getContext(),
					serviceClass);
			ComponentName componentName = NgnApplication.getContext()
					.startService(
							new Intent(NgnApplication.getContext(),
									getNativeServiceClass()));
	...
	/**
	 * Sets the main activity to use as context in order to query some native
	 * resources. It's up to you to call this function in order to retrieve the
	 * contacts for the ContactService.
	 * 
	 * @param mainActivity
	 *            The activity
